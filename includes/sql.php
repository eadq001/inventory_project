...
/*--------------------------------------------------------------*/
  ...
  /*--------------------------------------------------------------*/
   function page_require_level($require_level){
     ...
     if (!$session->isUserLoggedIn(true)):
      ...
      //if Group not found or status Deactive
 ...
 function find_recent_product_added($limit){
   global $db;
   $sql   = "SELECT p.id, p.name, p.sale_price, p.date, c.name AS categorie ";
   $sql  .= "FROM products p ";
   $sql  .= "LEFT JOIN categories c ON c.id = p.categorie_id ";
...
function find_sale_by_dates($start_date,$end_date){
  ...
  $sql  = "SELECT s.*, p.name, p.categorie_id, c.name AS category FROM sales s ";
  $sql .= "LEFT JOIN products p ON s.product_id = p.id ";
  $sql .= "LEFT JOIN categories c ON p.categorie_id = c.id ";
  $sql .= "WHERE STR_TO_DATE(s.date, '%m-%d-%Y') BETWEEN '{$start_date}' AND '{$end_date}' ";
  $sql .= "ORDER BY STR_TO_DATE(s.date, '%m-%d-%Y') DESC";
  return find_by_sql($sql);
}
/*--------------------------------------------------------------*/
...
function  dailySales($year,$month){
  global $db;
  $sql  = "SELECT s.id, s.qty, s.category, s.sizes, s.remarks, DATE_FORMAT(s.date, '%Y-%m-%e') AS date, p.name, SUM(p.sale_price * s.qty) AS total_saleing_price ";
  $sql .= "FROM sales s ";
  $sql .= "LEFT JOIN products p ON s.product_id = p.id ";
